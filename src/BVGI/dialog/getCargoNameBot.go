...
type GetCargoNameBot struct {
	fbbot.BaseStep
}
func (s GetCargoNameBot) Enter(bot *fbbot.Bot, msg *fbbot.Message) fbbot.Event {
	bot.SendText(msg.Sender, util.Personalize(T("getVesselName_title"), &msg.Sender))
	return StayEvent
}

func (s GetCargoNameBot) Process(bot *fbbot.Bot, msg *fbbot.Message) fbbot.Event {
	...
	////reg, _ := regexp.Compile("[^0-9-%áàảãạăắằẳẵặâấầẩẫậéèẻẽẹêếềểễệíìỉĩịóòỏõọôốồổỗộơớờởỡợúùủũụưứừửữựýỳỷỹỵđÁÀẢÃẠĂẮẰẲẴẶÂẤẦẨẪẬÉÈẺẼẸÊẾỀỂỄỆÍÌỈĨỊÓÒỎÕỌÔỐỒỔỖỘƠỚỜỞỠỢÚÙỦŨỤƯỨỪỬỮỰÝỲỶỸỴĐ]+")
	////if err != nil {
	////	log.Fatal(err)
	////}
	//prc_Str:= reg.ReplaceAllString(gOUTPUT, " ")
	//fmt.Print("prc_Str:"+prc_Str)
	...
	//}
	//fmt.Print("v_processedString:"+v_processedString)



	result, _ :=  soap.GetCargoNameInfo(Vessel_name)
	if (result==nil) {
		bot.SendText(msg.Sender, util.Personalize(T("NoData_Found") + "" , &msg.Sender))
		return ConfirmInputCargoNameEvent
	}
	// Read xml
