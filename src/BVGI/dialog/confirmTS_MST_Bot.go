...
type ConfirmTS_MST_Bot struct {
	fbbot.BaseStep
}

func (s ConfirmTS_MST_Bot) Enter(bot *fbbot.Bot, msg *fbbot.Message) fbbot.Event {
...
func (s ConfirmTS_MST_Bot) Process(bot *fbbot.Bot, msg *fbbot.Message) fbbot.Event {
	fmt.Print(strings.ToLower(util.RemoveAccent(msg.Text)))

	switch strings.ToLower(util.RemoveAccent(msg.Text)) {
	case strings.ToLower(util.RemoveAccent(T("confirmContinue_btn"))):
