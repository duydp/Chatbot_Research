...
type GetRegNumberCargoBot struct {
	fbbot.BaseStep
}
func (s GetRegNumberCargoBot) Enter(bot *fbbot.Bot, msg *fbbot.Message) fbbot.Event {
	bot.SendText(msg.Sender, util.Personalize(T("getMaSoDK_title"), &msg.Sender))
	return StayEvent
}

func (s GetRegNumberCargoBot) Process(bot *fbbot.Bot, msg *fbbot.Message) fbbot.Event {
	...
	strParametername=" |"+string(util.Personalize(T("getMaSoDK_title"), &msg.Sender))+":"
	soap.AppendStringToFileServer(strIDSender,strFileName,strParametername,msg.Text)

	var RegNumberVessel =strings.ToUpper(msg.Text)
	//var RegNumberVessel ="`"+msg.Text+"`"
	//RegNumberVessel=strings.Replace(strconv.Quote(strings.ToUpper(RegNumberVessel)), `"`, " ", 2)
	//
	//pr := transform.Chain(norm.NFD, runes.Remove(runes.In(unicode.Mn)), norm.NFC)
	//gOUTPUT, _, _ := transform.String(pr ,RegNumberVessel)
	...
	//}
	result, _ :=  soap.GetRegNumberCargoInfo(RegNumberVessel)
	if (result==nil) {
		bot.SendText(msg.Sender, util.Personalize(T("NoData_Found") + "" , &msg.Sender))
		return ConfirmInputregNumberCargoEvent
	}
	...
	for i := 0; i < len(rowSets.Rows); i++ {
		if len(rowSets.Rows)%1==0{
			...
			if rowSets.Rows[i].CERTIFICATENUMBER!=""{
				msgText+=`, GCNBH số ` + rowSets.Rows[i].CERTIFICATENUMBER
			}
