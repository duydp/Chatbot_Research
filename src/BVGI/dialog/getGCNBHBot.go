...
type GetGCNBHBot struct {
	fbbot.BaseStep
}
func (s GetGCNBHBot) Enter(bot *fbbot.Bot, msg *fbbot.Message) fbbot.Event {
	bot.SendText(msg.Sender, util.Personalize(T("getSoGCNBH_title"), &msg.Sender))
	return StayEvent
}

func (s GetGCNBHBot) Process(bot *fbbot.Bot, msg *fbbot.Message) fbbot.Event {
	...
	strFileName="Car.log"
	strParametername=" |"+string(util.Personalize(T("getSoGCNBH_title"), &msg.Sender))+":"
	soap.AppendStringToFileServer(strIDSender,strFileName,strParametername,msg.Text)
	var GCNBH =strings.ToUpper(msg.Text)

	//var GCNBH ="`"+msg.Text+"`"
	//GCNBH=strings.Replace(strconv.Quote(strings.ToUpper(GCNBH)), `"`, " ", 2)
	//
	//pr := transform.Chain(norm.NFD, runes.Remove(runes.In(unicode.Mn)), norm.NFC)
	//gOUTPUT, _, _ := transform.String(pr ,GCNBH)
