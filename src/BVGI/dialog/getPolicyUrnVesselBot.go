...
type GetPolicyURNVesselBot struct {
	fbbot.BaseStep
}
func (s GetPolicyURNVesselBot) Enter(bot *fbbot.Bot, msg *fbbot.Message) fbbot.Event {
	bot.SendText(msg.Sender, util.Personalize(T("getSoHopDong_title"), &msg.Sender))
	return StayEvent
}

func (s GetPolicyURNVesselBot) Process(bot *fbbot.Bot, msg *fbbot.Message) fbbot.Event {
	...
	var policyURN_Vessel =strings.ToUpper(msg.Text)
	//var policyURN_Vessel ="`"+msg.Text+"`"
	//policyURN_Vessel=strings.Replace(strconv.Quote(strings.ToUpper(policyURN_Vessel)), `"`, " ", 2)
	//
	//pr := transform.Chain(norm.NFD, runes.Remove(runes.In(unicode.Mn)), norm.NFC)
	//gOUTPUT, _, _ := transform.String(pr ,policyURN_Vessel)
	...
	//}
	result, _ :=  soap.GetPolicyURNVesselInfo(policyURN_Vessel)
	if (result==nil) {
