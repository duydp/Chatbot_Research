...
type GetPolicyHolderNameFireBot struct {
	fbbot.BaseStep
}
func (s GetPolicyHolderNameFireBot) Enter(bot *fbbot.Bot, msg *fbbot.Message) fbbot.Event {
	bot.SendText(msg.Sender, util.Personalize(T("getPolicyHolderName_title"), &msg.Sender))
	return StayEvent
}
func (s GetPolicyHolderNameFireBot) Process(bot *fbbot.Bot, msg *fbbot.Message) fbbot.Event {
	...
	var PolicyHolderName_Fire =strings.ToUpper(msg.Text)
	//var PolicyHolderName_Fire ="`"+msg.Text+"`"
	//PolicyHolderName_Fire=strings.Replace(strconv.Quote(strings.ToUpper(PolicyHolderName_Fire)), `"`, " ", 2)
	//
	//pr := transform.Chain(norm.NFD, runes.Remove(runes.In(unicode.Mn)), norm.NFC)
	//gOUTPUT, _, _ := transform.String(pr ,PolicyHolderName_Fire)
	...
	//}
	result, _ :=  soap.GetPolicyHolderNameFireInfo(PolicyHolderName_Fire)
	if (result==nil) {
