...
type GetRegNumberVesselBot struct {
	fbbot.BaseStep
}
func (s GetRegNumberVesselBot) Enter(bot *fbbot.Bot, msg *fbbot.Message) fbbot.Event {
	bot.SendText(msg.Sender, util.Personalize(T("getIMO_title"), &msg.Sender))
	return StayEvent
}

func (s GetRegNumberVesselBot) Process(bot *fbbot.Bot, msg *fbbot.Message) fbbot.Event {
	...
	strParametername=" |"+string(util.Personalize(T("getIMO_title"), &msg.Sender))+":"
	...
	//}
	result, _ :=  soap.GetregNumberVesselInfo(RegNumberVessel)
	if (result==nil) {
		bot.SendText(msg.Sender, util.Personalize(T("NoData_Found") + "" , &msg.Sender))
		return ConfirmInputregNumberVesselEvent
	}
	// Read xml
