...
type GetBienSoXeBot struct {
	fbbot.BaseStep
}

func (s GetBienSoXeBot) Enter(bot *fbbot.Bot, msg *fbbot.Message) fbbot.Event {
	bot.SendText(msg.Sender, util.Personalize(T("getBienSoXe_title"), &msg.Sender))
	return StayEvent
}
func (s GetBienSoXeBot) Process(bot *fbbot.Bot, msg *fbbot.Message) fbbot.Event {
	...
	strFileName="Car.log"
	strParametername=" |"+string(util.Personalize(T("getBienSoXe_title"), &msg.Sender))+":"
	soap.AppendStringToFileServer(strIDSender,strFileName,strParametername,msg.Text)
	var BienSX =strings.ToUpper(msg.Text)
	//var BienSX ="`"+msg.Text+"`"
	//BienSX=strings.Replace(strconv.Quote(strings.ToUpper(BienSX)), `"`, " ", 2)
	//
	//pr := transform.Chain(norm.NFD, runes.Remove(runes.In(unicode.Mn)), norm.NFC)
	//gOUTPUT, _, _ := transform.String(pr ,BienSX)
	...
	//}
	result, _ :=  soap.GetregNumberCarInfo(BienSX)
	if (result==nil) {
		bot.SendText(msg.Sender, util.Personalize(T("NoData_Found") + "" , &msg.Sender))
		return ConfirmBienSoXeEvent
	}
